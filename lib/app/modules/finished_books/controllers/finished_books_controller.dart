...
class FinishedBooksController extends AppController {
  final count = 0.obs;
  final inAsyncCall = false.obs;
  int load = 0;
  Map<String, dynamic> queryParametersForUserFinishedBooks = {};
  ...
  Future<void> onInit() async {
    super.onInit();
    screens = Screens.finishedBooks;
    inAsyncCall.value = true;
    if (await CM.internetConnectionCheckerMethod()) {
      try {
        await getUserFinishedBookApiCalling();
      } catch (e) {
  ...
  Future<void> getUserFinishedBookApiCalling() async {
    queryParametersForUserFinishedBooks = {
      ApiKey.limit: limit,
    ...
    http.Response? response = await HttpMethod.instance.getRequestForParams(
        ...
        endPointUri: UriConstant.endPointGetUserFinishedBooks,
        queryParameters: queryParametersForUserFinishedBooks);
    responseCode.value = response?.statusCode ?? 0;
    queryParametersForUserFinishedBooks.clear();
