...
class SearchSuggestionListController extends AppController {
  ...
  Future<void> getSearchBookDataApiCalling(text) async {
    if(text =='1')
    queryParametersForSearchBook = {
      //ApiKey.category: categoryValue,
      if(englishAssentValue.toString().isNotEmpty)
      ApiKey.englishAccent: englishAssentValue.toString(),
      if(levelValue.toString().isNotEmpty)
      ApiKey.level: levelValue.toString(),
      ////? ApiKey.language: languageValue,
      if(lengthValue.toString().isNotEmpty)
      ApiKey.length: lengthValue.toString(),
      ApiKey.bookName: searchController.text.toString().trim(),
      if(genreValue.toString().isNotEmpty)
      ApiKey.genre: genreValue.toString(),
    //  ApiKey.isAudio: isAudioValue.toString()
    };
    ...
    print(
        'this is search fitler data ' + " ${jsonDecode(response?.body ?? "")}");
    // print(" id value ${languageList[Filters.fromJson(json as Map<String, dynamic>).id!]}");
    print(
        " $categoryValue 1 $englishAssentValue 2 $levelValue 3 $languageValue 4 $lengthValue 5 ${searchController.text.toString().trim()}");
  ...
  Future<void> getGenreApiCalling() async {
    http.Response? response =
        await HttpMethod.instance.getRequest(url: UriConstant.endPointGetGenre);
    if (CM.responseCheckForGetMethod(response: response)) {
      ...
      if (getDataForGenre.value?.level != null) {
        getDataForGenre.value?.level?.forEach((element) {
          ...
          selectedGenre.add(-1);
          print("this is genre  ${genreList[index].id}");
  ...
  Future<void> getLanguageApiCalling() async {
    http.Response? response = await HttpMethod.instance
        .getRequest(url: UriConstant.endPointGetLanguage);
    if (CM.responseCheckForGetMethod(response: response)) {
      getDataForCategory.value =
          GetDashBoardBooksModel.fromJson(jsonDecode(response?.body ?? ""));
      if (getDataForCategory.value?.language != null) {
        ...
        getDataForCategory.value?.language?.forEach((element) {
          languageList.add(
            Filters(name: myaudioList[index], id: index ),
          );
  ...
  Future<void> getLengthApiCalling() async {
    http.Response? response = await HttpMethod.instance
        .getRequest(url: UriConstant.endPointGetLength);
    if (CM.responseCheckForGetMethod(response: response)) {
      getDataForCategory.value =
          GetDashBoardBooksModel.fromJson(jsonDecode(response?.body ?? ""));
...
void clickOnShowAll() async{
  ...
  onChange({required String value}) {
    ...
    searchOnStoppedTyping = Timer(duration, () async {
      await getSearchBookDataApiCalling("1");
    });
  }
  ...
  Future<void> clickOnParticularFilter(
    ...
    if (key == 0) {
      ...
      print(
          "${selectedGenre[index]} == ${genreList[index].id} && ${genre.contains(selectedGenre[index])}");

      if ((selectedGenre[index] == genreList[index].id) &&
          genre.contains(selectedGenre[index])) {
        genre.remove(selectedGenre[index]);

        selectedGenre[index] = -1;
      } else {
        selectedGenre[index] = genreList[index].id ?? -1;
        genre.add(genreList[index].id ?? -1);
      }
    } else if (key == 2) {
    ...
    genreValue = genre.toString().replaceAll("]", "");
    genreValue = genreValue.toString().replaceAll("[", "");
  ...
  void clearFilter() {
    ...
    for (var element in selectedGenre) {
      selectedGenre[i] = -1;
      i++;
    }
    // ignore: unused_local_variable
